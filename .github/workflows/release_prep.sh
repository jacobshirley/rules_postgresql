...
PREFIX="rules_postgresql-${TAG:1}"
ARCHIVE="rules_postgresql-$TAG.tar.gz"
...
\`\`\`starlark
bazel_dep(name = "rules_postgresql", version = "${TAG:1}")
\`\`\`
...
http_archive(
    ...
    sha256 = "${SHA}",
    strip_prefix = "${PREFIX}",
    url = "https://github.com/jacobshirley/rules_postgresql/releases/download/${TAG}/${ARCHIVE}",
)
EOF
