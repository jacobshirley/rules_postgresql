...
def _postgresql_repo_impl(repository_ctx):
    url = "https://github.com/someorg/someproject/releases/download/v{0}/postgresql-{1}.zip".format(
        repository_ctx.attr.postgresql_version,
        repository_ctx.attr.platform,
    )
    repository_ctx.download_and_extract(
        url = url,
        integrity = TOOL_VERSIONS[repository_ctx.attr.postgresql_version][repository_ctx.attr.platform],
    )
    build_content = """#Generated by postgresql/repositories.bzl
load("@rules_postgresql//postgresql:toolchain.bzl", "postgresql_toolchain")
postgresql_toolchain(name = "postgresql_toolchain", target_tool = select({
        "@bazel_tools//src/conditions:host_windows": "postgresql_tool.exe",
        "//conditions:default": "postgresql_tool",
    }),
)
"""
